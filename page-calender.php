...
for($x=0; $x<3; $x++){
	...
	echo '<table class="cal js-cal-list" id="js-cal-table-'.$x.'">';
	//該当月の年月表示
	echo '<tr>';
	if($x != 0){
		$set_x = $x -1;
	...
	for($i=1; $i<=$end_day; $i++){
		...
		if($week_date == 0){
			//日曜日
			echo '<td data-date="'.$set_date.'" class="sun ng  '.$offDay.'">'.$i.'</td>';
		}else if($week_date == 6){
			//土曜日
			echo '<td data-date="'.$set_date.'" class="sat ng '.$offDay.'">'.$i.'</td>';
		}else if(array_key_exists($set_date,$syuku_array)){
			//祝日
			echo '<td data-date="'.$set_date.'" class="sun ng '.$offDay.'">'.$i.'</td>';
		}else if($now_date_ymd > $set_date_ymd){
			//過去日付はNG
			echo '<td data-date="'.$set_date.'" class="ng'.$pastDay.' '.$nawDay .'">'.$i.'</td>';
		}else{
			//平日
			echo '<td data-date="'.$set_date.'" class="ok ddd">'.$i.'</td>';
		}
