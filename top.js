...
jQuery("#format.select__zone label").change(function () {

    //選択したボックスにスタイリング
    jQuery("#format.select__zone label").not(this).removeClass("checked");

    jQuery(this).addClass("checked");
});

jQuery("#limit.select__btn__zone label").change(function () {

    //選択したボックスにスタイリング
    jQuery("#limit.select__btn__zone label").not(this).removeClass("checked");

    jQuery(this).addClass("checked");
});

jQuery("#data.select__btn__zone label").change(function () {

    //選択したボックスにスタイリング
    jQuery("#data.select__btn__zone label").not(this).removeClass("checked");

    jQuery(this).addClass("checked");
});

jQuery("#publish.select__btn__zone label").change(function () {

    //選択したボックスにスタイリング
    jQuery("#publish.select__btn__zone label").not(this).removeClass("checked");
...
jQuery(function () {
    ...
    btn.on("click", function () {
        ...
        var label5 = jQuery("#size__select > .label5").text();
        //クリックしたボタンの管理番号を入力
        jQuery("#output__type").text(label5);
        jQuery("#control__num").css("display", "block");
